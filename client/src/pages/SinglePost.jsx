  ...
  return (
    <CardContainer>
      ...
      {clubPost.coordinators.map((cor, key) => {
        const name = typeof cor === 'string' ? cor : cor.name;
        const email = typeof cor === 'string' ? '' : cor.email;
        const phone = typeof cor === 'string' ? '' : cor.phone;
        return (
          <CoordinatorInfoContainer key={key}>
            <p>Contact {key + 1}: </p>
            <p>{name}</p>
            {email && <p>{email}</p>}
            {phone && <p>{phone}</p>}
